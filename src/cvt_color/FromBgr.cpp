...
{
    uint8_t m_y[3]{38, 75, 15};                //  yr,  yg,  yb
    uint16_t m_uv[6]{22, 42, 64, 64, 54, 10};  // -ur, -ug,  ub
} k_yuv_param;
...
void bgr_to_gray(const uint8_t *src, int32_t w, int32_t h, uint8_t *dst)
{
...
void bgr_to_rgba(const uint8_t *src, int32_t w, int32_t h, uint8_t *dst)
{
    auto size = k_pixel_size * w * h;
...
void bgr_to_rgb(const uint8_t *src, int32_t w, int32_t h, uint8_t *dst)
{
    auto size = k_pixel_size * w * h;
...
void bgr_to_bgra(const uint8_t *src, int32_t w, int32_t h, uint8_t *dst)
{
    auto size = k_pixel_size * w * h;
    for (auto i = 0L, j = 0L; i < size; i += k_pixel_size, j += 4L) {
        dst[j]     = src[i];
...
void bgr_to_bgr(const uint8_t *src, int32_t w, int32_t h, uint8_t *dst)
{
    auto size = k_pixel_size * w * h;
    std::copy_n(src, size, dst);
}

void bgr_to_yuyv(const uint8_t *src, int32_t w, int32_t h, uint8_t *dst)
{
    auto size               = 1L * w * h;
    constexpr auto k_offset = 1U << k_shift;
    for (auto i = 0L; i + 2 < size; i += 2) {
        auto u0 = (src[(i * k_pixel_size) + 0] * k_yuv_param.m_uv[2] - src[(i * k_pixel_size) + 1] * k_yuv_param.m_uv[1] - src[(i * k_pixel_size) + 2] * k_yuv_param.m_uv[0]) >> k_shift;
        auto v0 = (src[(i * k_pixel_size) + 2] * k_yuv_param.m_uv[3] - src[(i * k_pixel_size) + 1] * k_yuv_param.m_uv[4] - src[(i * k_pixel_size) + 0] * k_yuv_param.m_uv[5]) >> k_shift;

        auto u1 = (src[(i * k_pixel_size) + k_pixel_size + 0] * k_yuv_param.m_uv[2] - src[(i * k_pixel_size) + k_pixel_size + 1] * k_yuv_param.m_uv[1] - src[(i * k_pixel_size) + k_pixel_size + 2] * k_yuv_param.m_uv[0]) >> k_shift;
        auto v1 = (src[(i * k_pixel_size) + k_pixel_size + 2] * k_yuv_param.m_uv[3] - src[(i * k_pixel_size) + k_pixel_size + 1] * k_yuv_param.m_uv[4] - src[(i * k_pixel_size) + k_pixel_size + 0] * k_yuv_param.m_uv[5]) >> k_shift;

        dst[(i << 1) + 0] = (src[(i * k_pixel_size)] * k_yuv_param.m_y[2] + src[(i * k_pixel_size) + 1] * k_yuv_param.m_y[1] + src[(i * k_pixel_size) + 2] * k_yuv_param.m_y[0]) >> k_shift;
        dst[(i << 1) + 1] = ((u0 + u1) >> 1) + k_offset;
        dst[(i << 1) + 2] = (src[(i * k_pixel_size) + k_pixel_size] * k_yuv_param.m_y[2] + src[(i * k_pixel_size) + k_pixel_size + 1] * k_yuv_param.m_y[1] + src[(i * k_pixel_size) + k_pixel_size + 2] * k_yuv_param.m_y[0]) >> k_shift;
        dst[(i << 1) + 3] = ((v0 + v1) >> 1) + k_offset;
    }
}

void bgr_to_uyvy(const uint8_t *src, int32_t w, int32_t h, uint8_t *dst)
{
    auto size               = 1L * w * h;
    constexpr auto k_offset = 1U << k_shift;
    for (auto i = 0L; i + 2 < size; i += 2) {
        ...
        dst[(i << 1) + 1] = (src[(i * k_pixel_size)] * k_yuv_param.m_y[2] + src[(i * k_pixel_size) + 1] * k_yuv_param.m_y[1] + src[(i * k_pixel_size) + 2] * k_yuv_param.m_y[0]) >> k_shift;
        dst[(i << 1) + 2] = ((v0 + v1) >> 1) + k_offset;
        dst[(i << 1) + 3] = (src[(i * k_pixel_size) + k_pixel_size] * k_yuv_param.m_y[2] + src[(i * k_pixel_size) + k_pixel_size + 1] * k_yuv_param.m_y[1] + src[(i * k_pixel_size) + k_pixel_size + 2] * k_yuv_param.m_y[0]) >> k_shift;
    }
}

void bgr_to_i420(const uint8_t *src, int32_t w, int32_t h, uint8_t *dst)
{
    auto size = k_pixel_size * w * h;
...
void bgr_to_nv12(const uint8_t *src, int32_t w, int32_t h, uint8_t *dst)
{
    auto size = k_pixel_size * w * h;
    auto ss   = k_pixel_size * w;
    auto ys   = w;
...
void bgr_to_nv21(const uint8_t *src, int32_t w, int32_t h, uint8_t *dst)
{
    auto size = k_pixel_size * w * h;
    auto ss   = k_pixel_size * w;
    auto ys   = w;
...
void bgr_to_gray_asm(const uint8_t *src, int32_t w, int32_t h, uint8_t *dst)
{
    asm volatile(
        LOAD_Y_PARAM
        R"(
        GRAY_L32:
            ...
            umull      v3.8h, v0.8b, v31.8b    // * b
            umlal      v3.8h, v1.8b, v30.8b    // * g
            umlal      v3.8h, v2.8b, v29.8b    // * r
            prfm    pldl1keep, [%0, 448]
            umull2     v7.8h, v0.16b, v31.16b  // * b
            umlal2     v7.8h, v1.16b, v30.16b  // * g
            umlal2     v7.8h, v2.16b, v29.16b  // * r
            uqshrn     v0.8b, v3.8h, %[shift]
            uqshrn2    v0.16b, v7.8h, %[shift]
            umull      v2.8h, v4.8b, v31.8b    // * b
            umlal      v2.8h, v5.8b, v30.8b    // * g
            umlal      v2.8h, v6.8b, v29.8b    // * r
            umull2     v8.8h, v4.16b, v31.16b  // * b
            umlal2     v8.8h, v5.16b, v30.16b  // * g
            umlal2     v8.8h, v6.16b, v29.16b  // * r
        ...
        GRAY_L16:
            ...
            umlal     v3.8h, v1.8b, v30.8b    // * g
            umlal     v3.8h, v2.8b, v29.8b    // * r
            umull2    v4.8h, v0.16b, v31.16b  // * b
            umlal2    v4.8h, v1.16b, v30.16b  // * g
            umlal2    v4.8h, v2.16b, v29.16b  // * r
            uqshrn    v0.8b, v3.8h, %[shift]
        ...
        : "+r"(src), "+r"(size), "+r"(dst)
        : [param] "r"(&k_param), [shift] "i"(k_shift)
        : "memory", "cc", "v0", "v1", "v2", "v3", "v4", "v5", "v6", "v7", "v8", USED_Y_REG);
}

void bgr_to_rgba_asm(const uint8_t *src, int32_t w, int32_t h, uint8_t *dst)
{
    int64_t size = 1L * w * h;
    asm volatile(
        R"(
...
void bgr_to_rgb_asm(const uint8_t *src, int32_t w, int32_t h, uint8_t *dst)
{
    auto size = 1L * w * h;
    ...
    asm volatile(
        ...
        RGB_L4:
            ...
            mov v0.8b, v2.8b
            mov v2.8b, v8.8b
            st3       {v0.s, v1.s, v2.s}[0], [%2], #12

        RGB_L2:
            cmp       %1, #2
            blt       RGB_L1
            sub       %1, %1, #2
            ld3       {v0.h, v1.h, v2.h}[0], [%0], #6
            mov v8.8b, v0.8b
            mov v0.8b, v2.8b
            mov v2.8b, v8.8b
            st3       {v0.h, v1.h, v2.h}[0], [%2], #6
...
void bgr_to_bgr_asm(const uint8_t *src, int32_t w, int32_t h, uint8_t *dst)
{
    auto size = 1L * w * h;
    asm volatile(
        R"(
...
void bgr_to_bgra_asm(const uint8_t *src, int32_t w, int32_t h, uint8_t *dst)
{
    auto size = 1L * w * h;
    asm volatile(
        R"(
...
void bgr_to_yuyv_asm(const uint8_t *src, int32_t w, int32_t h, uint8_t *dst)
{
    auto size = 1L * w * h;
    asm volatile(
        LOAD_YUV_PARAM
...
void bgr_to_uyvy_asm(const uint8_t *src, int32_t w, int32_t h, uint8_t *dst)
{
    auto size = 1L * w * h;
    asm volatile(
        LOAD_YUV_PARAM
...
void bgr_to_i420_asm(const uint8_t *src, int32_t w, int32_t h, uint8_t *dst)
{
    auto y = dst;
    ...
    asm volatile(
        ...
        I420_2H_L2:
            ...
            ld3 {v0.b, v1.b, v2.b}[0], [%0], #3
            ld3 {v0.b, v1.b, v2.b}[1], [%0], #3
            ld3 {v4.b, v5.b, v6.b}[0], [x0], #3
            ld3 {v4.b, v5.b, v6.b}[1], [x0], #3
...
void bgr_to_nv12_asm(const uint8_t *src, int32_t w, int32_t h, uint8_t *dst)
{
    auto y  = dst;
...
void bgr_to_nv21_asm(const uint8_t *src, int32_t w, int32_t h, uint8_t *dst)
{
    auto y  = dst;
    auto uv = dst + static_cast<ptrdiff_t>(w * h);
    asm volatile(
